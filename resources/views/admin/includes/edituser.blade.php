    ...
    <div class="mx-2">
      ...
      <form action="{{route('user.update',$user->id)}}" method="post" class="px-md-5">
        ...
        <div class="form-group mb-3 row">
          ...
          <div class="col-md-10">
            <input type="password" placeholder="Password" class="form-control py-2" name="password"  value="{{old('password',$user->password)}}"/>
            @error('password')
                <div class="alert alert-warning">{{$message}}</div>
            @enderror
          </div>
        ...
        <div class="form-group mb-3 row">
          ...
          <div class="col-md-10">
            <input type="checkbox" class="form-check-input" style="padding: 0.7rem;" name="active"@checked(old('active',$user->active)) value="1" />
            @error('active')
                <div class="alert alert-warning">{{$message}}</div>
            @enderror
          </div>
